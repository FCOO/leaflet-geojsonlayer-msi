    ...
    this._layers = {};
      ...
      this.options.onEachFeature = function (feature, layer) { 
        // Use click handler for substituting timezone information
				var point_template, popup_template;
				layer.on({
          click: function (evt) {
            point_template = '<p>Position: {latitude}&nbsp;&nbsp;{longitude}</p>';
            if (feature.properties.language == 'da') {
              popup_template = '<div class="msi"><h4>Aktuelle advarsler</h4><p>{body}</p><hr/><p>Tid: {updated}</p><hr/><p>Hovedområde: {mainarea}</p><p>Underområde: {subarea}</p><hr/>{points}<hr/><p>Kilde: <a target="_new" href="http://www.soefartsstyrelsen.dk">Søfartsstyrelsen</a></p></div>';
            } else {
              popup_template = '<div class="msi"><h4>Maritime Safety Information</h4><p>{body}</p><hr/><p>Time: {updated}</p><hr/><p>Main area: {mainarea}</p><p>Subarea: {subarea}</p><hr/>{points}<hr/><p>Source: <a target="_new" href="http://dma.dk">Danish Maritime Authority</a></p></div>';
            }
